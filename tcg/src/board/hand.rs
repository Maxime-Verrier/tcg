...
use card_sim::agent_action::ClientJoinedBoardPacket;
...
pub enum OnHandSource {
    Board(Entity),
    Hand(Entity),
    Deck(Entity),
}

pub struct OnHandModified {
    pub agent: Entity,
    pub board: Entity,
    pub inserteds: HashMap<OnHandSource, Entity>,
}
...
pub(crate) fn on_client_join_board_render(
    ...
    use crate::board::client_action::{ClientAction, ClientActionState, ClientSummonAction, SummonActionFinishEvent};
    ...
    for packet in packets.read() {
            if let Ok(board) = boards.get(packet.board) {
                ...
                for (hand_agent, hand) in hands {
                    ...
                    println!("hand_agent: {:?}, packet.agent: {:?}", hand_agent, packet.agent);

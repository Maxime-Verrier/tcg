#[cfg(feature = "render")]
pub use cgf_card_render::*;

#[cfg(feature = "render")]
mod cgf_card_render {
    use bevy::prelude::*;
    use bevy::{ecs::system::EntityCommands, utils::HashMap};
    use card_sim::{Card, CardAttribute, CardId};
    ...
    pub fn create_card_render(
        ...
        if let Ok((card, attribute, transform, visibility)) = cards.get(card_entity.0) {
            ...
            error!(
                "Could not create card render, the provided entity do not have a card component"
            );
        }
    }
